...
u_indepSensiVarsC2 = c("E_CIm", "E_CEx", "E_CP", 
                       ...
                       "EE", 
                       "GDP_Ag", "GDP_Ind", "GDP_Ser", "GDP_Tra")
u_indepSensiVarsC2 = c("E_CCGDP", "GDPpc",
                       "P")
#"E_CRes", "E_CSeInd", 
#"E_OC", "E_GC", "E_NC", 
#"E_RenC", 
...
defineCases  <- function() {list(
  #===== Config2 ================================================================================
  #===== E_CC  ==================================================================================
  "testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - maxdepth5" = list(
  ...
  "testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - maxdepth7" = list(
    vardef    = generate_variableDefinition(v_dataShort, d="E_CC", n=u_indepSensiVarsC2),
    period    = u_periodC2,
    iso       = u_iso_sensiVarC2,
    dvar      = "E_CC",
  ...
  "testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - maxdepth10" = list(
    vardef    = generate_variableDefinition(v_dataShort, d="E_CC", n=u_indepSensiVarsC2),
    period    = u_periodC2,
    iso       = u_iso_sensiVarC2,
    dvar      = "E_CC",
    ...
    firstdiff = FALSE,
    treetype  = "Single tree > LMS - Multilinear",
    maxsplits = 10, minnbnodes = 3,
    cex       = 0.3, legend.cex = 1.5
  ),
  "testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - maxdepth13" = list(
    vardef    = generate_variableDefinition(v_dataShort, d="E_CC", n=u_indepSensiVarsC2),
    period    = u_periodC2,
    iso       = u_iso_sensiVarC2,
    dvar      = "E_CC",
    ...
    firstdiff = FALSE,
    treetype  = "Single tree > LMS - Multilinear",
    maxsplits = 13, minnbnodes = 3,
    cex       = 0.3, legend.cex = 1.5
  ),
  "testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - maxdepth5 - sp1y" = list(
    vardef    = generate_variableDefinition(v_dataShort, d="E_CC", n=u_indepSensiVarsC2),
    period    = c(1992,2012),
    iso       = u_iso_sensiVarC2,
    dvar      = "E_CC",
  ...
  "testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - maxdepth7 - sp1y" = list(
    vardef    = generate_variableDefinition(v_dataShort, d="E_CC", n=u_indepSensiVarsC2),
    period    = c(1992,2012),
    iso       = u_iso_sensiVarC2,
    dvar      = "E_CC",
  ...
  "testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - maxdepth10 - sp1y" = list(
    vardef    = generate_variableDefinition(v_dataShort, d="E_CC", n=u_indepSensiVarsC2),
    period    = c(1992,2012),
    iso       = u_iso_sensiVarC2,
    dvar      = "E_CC",
  ...
  "testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - maxdepth13 - sp1y" = list(
    vardef    = generate_variableDefinition(v_dataShort, d="E_CC", n=u_indepSensiVarsC2),
    period    = c(1992,2012),
    iso       = u_iso_sensiVarC2,
    dvar      = "E_CC",
...
for (k_case in names(u_cases)) {
  ...
  outPath  <- file.path("output/testingMethods3/RT models", k_case)
...
# dir.create(file.path("output/testingMethods3/data"))
# save(u_cases, v_data, v_tree, v_alloc, v_regdata, file = file.path("output/testingMethods3/data", paste0("testing_methods3_", format(Sys.time(), "%Y-%m-%d"),".RData")))
load(file.path("output/testingMethods3/data", paste0("testing_methods3_", format(Sys.time(), "%Y-%m-%d"),".RData")))


#==== Plot results ==============
dir.create(file.path("output/testingMethods3/Plots Trees"))
dir.create(file.path("output/testingMethods3/Plots country pathways"))
dir.create(file.path("output/testingMethods3/Pred Plots"))
  ...
  if (!is.null(v_tree[[k_case]])) {
    ...
    if (dim(tree$frame)[1] != 0) {
      plot_tree(tree, 
                ...
                FILENAME = file.path("output/testingMethods3/Plots Trees/", paste0(k_case, ".pdf")))
      plot_tree(tree, 
                TITLE=paste0("RT - ", k_case, "\nLMS-Multilinear - Tn:", length(which(v_tree[[k_case]]$nodeType == "Terminal node")), "\n#Countries: ",length(unique(v_data[[k_case]]$data$iso)), " - period: ",min(v_data[[k_case]]$data$year),"-",max(v_data[[k_case]]$data$year)), 
                CEX=NULL, LEGEND.X = 0, LEGEND.Y = 1.1, LEGEND.CEX=0.5, LEGEND.NCOL=2,
                FILENAME = file.path("output/testingMethods3/Plots Trees/", paste0(k_case, ".png")))
    }
    
    if (dim(v_alloc[[k_case]])[1] != 0) {
      # Country pathways
      png(filename=file.path("output/testingMethods3/Plots country pathways/", paste0(k_case, ".png")), width=1500, height=1500)
      ...
      #print(p)
      ggsave(filename = file.path("output/testingMethods3/Pred Plots/", paste0(k_case, ".png")), width = 297, height = 210, units = c("mm"), dpi = 300)
    }
  }
}
...
colnames(tmp) <- gsub("testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - ", "", names(u_cases))


tmp <- tmp %>% 
    as.data.frame() %>% 
    mutate(T1=gsub("testingMethods3 - Fixed effects - SensiVarsC2 - E_CC - ", "", names(u_cases))) %>% 
    gather(T2, value, -T1) %>% 
    mutate(T1=factor(T1, levels=c("maxdepth5","maxdepth7","maxdepth10","maxdepth13", "maxdepth5 - sp1y","maxdepth7 - sp1y","maxdepth10 - sp1y","maxdepth13 - sp1y"), ordered=TRUE)) %>% 
    mutate(T2=factor(T2, levels=c("maxdepth5","maxdepth7","maxdepth10","maxdepth13", "maxdepth5 - sp1y","maxdepth7 - sp1y","maxdepth10 - sp1y","maxdepth13 - sp1y"), ordered=TRUE))
